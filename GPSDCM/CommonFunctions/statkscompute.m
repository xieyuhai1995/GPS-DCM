...
if isequal(ftype,'icdf')
    ...
    yData = transform(yData,L,U);
    foldpoint = transform(foldpoint,L,U);

    % Compute on that scale
    [fout,xout]=compute_icdf(xi,xispecified,m,u,weight,cutoff,...
                             kernelname,ty,yData,foldpoint,maxp);
...
ximin = untransform(ximin,L,U);
ximax = untransform(ximax,L,U);
...
% -----------------------------
function [x1,p] = compute_icdf(xi,xispecified,m,u,...
    weight,cutoff,kernelname,ty,yData,foldpoint,maxp)
L = -Inf; % working on transformed scale, no bounds
U = Inf;
